...
parser.add_argument("--device", type=str, default="cpu", choices=["cpu", "cuda"], help='use cpu or cuda')
parser.add_argument("--data", type=str, default="p100", choices=["p100"], help='dataset')
parser.add_argument("--seed", type=int, default=100, help='random seed')
parser.add_argument("--eps", type=float, default=8/255, help='Target training epsilon')
parser.add_argument("--norm", type=float, default=2, help='p norm for epsilon perturbation')
parser.add_argument("--bound_type", type=str, default="CROWN-IBP",
                    choices=["IBP", "CROWN-IBP", "CROWN"], help='method of bound analysis')
parser.add_argument("--model", type=str, default="purchase_model",
...
parser.add_argument("--lr_decay_milestones", nargs='+', type=int, default=[70, 85], help='learning rate dacay milestones')
...
parser.add_argument("--scheduler_opts", type=str, default="start=10,length=61,mid=0.4", help='options for epsilon scheduler')
...
def main(args):
    ...
    if args.data == "p100":
        ...
        dummy_input = torch.randn(2, 600)
        train_data = PytorchDataset(x_target_train, y_target_train)
        test_data = PytorchDataset(x_target_test, y_target_test)
    elif args.data == 'MNIST':
        ...
        train_data = list(datasets.CIFAR10("./data", train=True, download=True,
                transform=transforms.Compose([transforms.ToTensor()])))[:2500]
        test_data = list(datasets.CIFAR10("./data", train=True, download=True,
                transform=transforms.Compose([transforms.ToTensor()])))[2500:5000]
    ...
    macs, params = profile(model_ori, (dummy_input,))
